...
export default function BottomTabBar() {
  const tabs = [
    ...
    { name: 'More', path: '/settings', icon: MenuRoundedIcon }, // Simple mapping to settings/menu for now
  ];
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) => cn(
                "flex flex-col items-center justify-center w-full h-full space-y-1 focus-ring rounded-xl",
                isActive ? "text-teal-800 dark:text-teal-100" : "text-teal-600 dark:text-teal-300",
            ...
            >
              {({ isActive }) => (
                <>
                  <div className={cn(
                    "relative flex items-center justify-center transition-transform duration-fast",
                    tab.isPrimary && "bg-mint-100 dark:bg-mint-900 rounded-full w-12 h-8",
                    isActive && !tab.isPrimary && "scale-110"
                  )}>
                    <Icon fontSize={tab.isPrimary ? "medium" : "small"} />
                    {isActive && !tab.isPrimary && (
                      <span className="absolute -bottom-2 w-1 h-1 rounded-full bg-current" />
                    )}
                  </div>
